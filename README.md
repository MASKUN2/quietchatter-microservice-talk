...
com.quietchatter.talk/
  ...
  domain/          Talk.kt, Reaction.kt, ReactionType.kt, TalkOwnershipService.kt, ForbiddenException.kt
  application/
...
| 메서드 | 경로 | 인증 | 설명 |
|---|---|---|---|
| GET    | /api/talks?memberId=&hidden=false | Optional | 특정 회원의 공개 북톡 목록 |
| GET    | /api/talks?memberId=&hidden=true | Required | 특정 회원의 숨겨진 북톡 목록 (본인만 조회 가능) |
...
| POST   | /api/talks/{talkId}/restore | Required | 숨겨진 북톡 복원 (작성자만) |
...
Talk: id(UUID), bookId, memberId, content, isHidden(Boolean), dateToHidden(LocalDate)

Reaction: id(UUID), talkId, memberId, type(ReactionType)

TalkOwnershipService: 행위자(Owner/Admin/System)별 북톡 상태 변경을 담당하는 도메인 서비스. hideByOwner/hideByAdmin/hideBySystem/restoreByOwner/updateContentByOwner/authorizeHiddenAccess 메서드를 제공하며, 소유권 불일치 시 ForbiddenException을 발생시킨다. ForbiddenException은 domain 패키지에 위치한다.
...
- 북톡 삭제는 물리 삭제가 아닌 isHidden=true 처리
- dateToHidden 기본값은 작성/수정/복원 시점으로부터 12개월 후로 설정됨
- 공개 목록 조회 시 isHidden=true인 북톡은 자동으로 제외
- 숨겨진 북톡 조회(hidden=true)는 작성자 본인만 가능 (불일치 시 403)
- 숨겨진 북톡 복원 시 isHidden=false, dateToHidden이 현재 시점으로부터 12개월 후로 재설정됨
- 수정/삭제/복원은 작성자 본인만 가능 (불일치 시 403)
