...
회원이 책에 대한 이야기(북톡)를 작성하고, 다른 사람의 북톡에 반응(좋아요 등)을 남길 수 있습니다.
회원이 책에 대한 이야기(북톡)를 작성하고, 다른 사람의 북톡에 회원 반응(좋아요 등)을 남길 수 있습니다. (회원 단위로 관리됨)
...
* `X-Member-Id` 헤더가 있으면: 해당 회원의 반응 여부(reacted)를 포함하여 응답합니다.
...
* 동일 회원이 동일 책에 북톡을 하나만 작성할 수 있습니다.
